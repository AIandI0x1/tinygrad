...
from tinygrad.runtime.support.system import RemotePCIDevice, APLRemotePCIDevice, RemoteCmd
...
class CPUProgram(Program['CPUDevice']):
  ...
  def __init__(self, dev:CPUDevice, obj:TinyELF):
    ...
    if dev.remote is not None and not getattr(dev.remote, 'exec_local', False):
      self.fxn:Any = dev.remote.rpc(RemoteCmd.LOAD_PROG, len(obj.lib), payload=obj.lib)[0]
  ...
  def __call__(self, *bufs:int, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1),
    ...
    if (remote:=self.dev.remote) is not None and not getattr(remote, 'exec_local', False):
    ...
    if remote is not None and getattr(remote, 'exec_local', False): cast(APLRemotePCIDevice, remote).flush_shadows()
...
class CPUDevice(Compiled):
  ...
  def __init__(self, device:str=""):
    self.remote:RemotePCIDevice|None = None
    if len(parts:=device.split(':')) >= 2 and parts[1] == "APL": self.remote = APLRemotePCIDevice("CPU", device[4:])
    elif len(parts) == 3:
  ...
  def synchronize(self, timeout:int|None=None):
    if self.remote is not None and not getattr(self.remote, 'exec_local', False): self.remote.rpc(RemoteCmd.PING)
